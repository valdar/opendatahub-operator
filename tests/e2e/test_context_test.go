...
func NewTestContext(t *testing.T) (*TestContext, error) { //nolint:thelper
	tcf, err := testf.NewTestContext(
		testf.WithTOptions(
			testf.WithEventuallyTimeout(gomegaEventuallyTimeout),
			testf.WithEventuallyPollingInterval(gomegaEventuallyPollInterval),
			testf.WithConsistentlyDuration(gomegaConsistentlyTimeout),
			testf.WithConsistentlyPollingInterval(gomegaConsistentlyPollInterval),
		),
	)
...
func (tc *TestContext) EnsureOperatorInstalled(nn types.NamespacedName, skipOperatorGroupCreation bool) {
	...
	}).WithTimeout(tc.TestTimeouts.eventuallyTimeoutMedium).WithPolling(tc.TestTimeouts.generalRetryInterval)
}
